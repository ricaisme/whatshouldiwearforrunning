...
import com.github.simonpercic.oklog.core.OkLogInterceptor;
...
//        interceptor.setLevel(HttpLoggingInterceptor.Level.BODY);
    	OkLogInterceptor interceptor = OkLogInterceptor.builder().useAndroidLog(true).build();
    ...
    public static void getWeatherGeoLocation(String lat, String lon, final OpenWeatherResponseCallback<OpenWeatherResponse> callback) {
        OpenWeatherMapService service = sRetrofit.create(OpenWeatherMapService.class);
        Call<OpenWeatherResponse> response = service.getWeatherFromGeoLocation(sAppId, lat, lon);
    ...
    public static void getUVIndexGeoLocation(String lat, String lon, final OpenWeatherResponseCallback<UVIndexResponse> callback) {
        OpenWeatherMapService service = sRetrofit.create(OpenWeatherMapService.class);
        
        Call<UVIndexResponse> response = service.getUVIndexFromGeoLocation( lat, lon,sAppId);
        response.enqueue(new Callback<UVIndexResponse>() {
            @Override
            public void onResponse(Call<UVIndexResponse> call, Response<UVIndexResponse> response) {
                ...
                if (response.isSuccessful()) {
                	UVIndexResponse result = response.body();
                    if (DEBUG) {
                        Log.d(TAG, "response.isSuccess() == true");
                        printResponse(result);
                    }
    ...
    public static void printResponse(UVIndexResponse result) {
        Log.i(TAG, "time:" + result.getTime());
        Log.i(TAG, "data:" + result.getData());
        Log.i(TAG, "location.lat:" + result.getLocation().getLatitude());
        Log.i(TAG, "location.lon:" + result.getLocation().getLongitude());
    }
